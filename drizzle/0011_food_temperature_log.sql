-- Food temperature log: cooking/reheating, hot holding, cold display, and
-- delivery checks (chilled + frozen), against UK Safer Food Better Business
-- limits. No staff sign-in on this page (same as the fridge/freezer checks
-- and daily checklist), so there is no staff attribution column.
...
-- The "Probe checked & calibrated today" chip: one real confirmation per
-- department per day, not just decorative.
