          ...
          <div className="top-actions">
            ...
            <div className="notification-wrap">
              ...
              {notificationsOpen && <div className="notification-popover glass-panel">
                ...
                {messages.filter((message) => message.unread).map((message) => <button key={message.id} className={`notification-item ${message.urgent ? 'urgent' : ''}`} onClick={() => setMessages((current) => current.map((item) => item.id === message.id ? { ...item, unread: false } : item))}><span className="notification-symbol">{message.urgent ? <Zap size={15} /> : <BellRing size={15} />}</span><span><strong>{message.from}</strong><small>{message.text}</small></span></button>)}
              </div>}
            </div>
